...
VAR_filelist <- fread(paste0("/data/MET_share/work/001_projects/proteins-crc/code/001_MR/004_MR/001_exposure-proteins_outcome-cancer/filelist/", VAR_filelist), header = F, sep = " ")
...
# data ====
cat("## data \n")
data_harmonise <- fread(paste0("/data/MET_share/work/001_projects/proteins-crc/analysis/001_MR/003_harmonise/001_exposure-proteins_outcome-cancer/", VAR_filelist, ".txt"))

# data VARS ====
VAR_exposure_samplesize <- data_harmonise %>%
  select(id.exposure, samplesize.exposure) %>%
  unique() %>%
  summarise(min_samplesize = min(samplesize.exposure, na.rm = TRUE)) %>%
  pull(min_samplesize)
...
data_mr <- data_mr %>%
  ...
  separate(
    ...
    sep = ";", remove = TRUE) %>%
  left_join(., VAR_outcome_samplesize, by = "id.outcome") %>%
  rename(outcome_samplesize = samplesize.outcome) %>%
  mutate(exposure_samplesize = VAR_exposure_samplesize) %>%
  select(
...
write.table(
  data_mr, 
  paste0("/data/MET_share/work/001_projects/proteins-crc/analysis/001_MR/004_MR/001_exposure-proteins_outcome-cancer/MR/", VAR_filelist, ".txt"), 
...
write.table(
  data_sensitivity, 
  paste0("/data/MET_share/work/001_projects/proteins-crc/analysis/001_MR/004_MR/001_exposure-proteins_outcome-cancer/singlesnp/", VAR_filelist, ".txt"), 
  row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
...
if (any(data_mr$nsnp > 3)) {
  ...
  write.table(
    data_sensitivity, 
    paste0("/data/MET_share/work/001_projects/proteins-crc/analysis/001_MR/004_MR/001_exposure-proteins_outcome-cancer/heterogeneity/", VAR_filelist, ".txt"), 
    row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
}
...
if (any(data_mr$nsnp > 3)) {
  ...
  write.table(
    data_sensitivity, 
    paste0("/data/MET_share/work/001_projects/proteins-crc/analysis/001_MR/004_MR/001_exposure-proteins_outcome-cancer/pleiotropy/", VAR_filelist, ".txt"), 
    row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
}
...
if (any(data_mr$nsnp > 3)) {
  ...
  write.table(
    data_sensitivity, 
    paste0("/data/MET_share/work/001_projects/proteins-crc/analysis/001_MR/004_MR/001_exposure-proteins_outcome-cancer/leaveoneout/", VAR_filelist, ".txt"), 
    row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
}
# END ====
cat("# FINISHED \n")
