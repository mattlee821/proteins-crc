...
# reference ====
reference_EUR <- data.table::fread(paste0("/data/GWAS_data/work/references/1000genomes/phase3/EUR/stats.stats"))
reference_ALL <- data.table::fread(paste0("/data/GWAS_data/work/references/1000genomes/phase3/ALL/stats.stats"))
...
tryCatch({
  if (!is.null(data_coloc)) {
    ...
    reference <- switch(VAR_population,
                        "EUR" = reference_EUR,
                        "ALL" = reference_ALL,
                        stop("Invalid value for VAR_population. Must be 'EUR' or 'ALL'.")
    )
  ...
  message("Error with alignment check and allele flipping", e$message)
  return(NULL)
})
# Skip if no significant p-values
...
for (i in seq_along(list_data)) {
  ...
  SNP_causal_exposure <- coloc::finemap.abf(dataset = data_coloc1) %>%
    ...
    select(snp, SNP.PP)
  if (any(SNP_causal_exposure$SNP.PP < 0.5, na.rm = TRUE)) {
    message("Skipping iteration: SNP.PP < 0.5 for either exposure or outcome.")
    next
  }
  ...
  for (k in seq_along(coloc_results)) {
    ...
    results <- data.frame(
      exposure = data_coloc1$phenotype,
      outcome = data_coloc2$phenotype,
  ...
  id_nsnp <- ifelse(abs(nsnps - 10000) < abs(nsnps - 5000) & abs(nsnps - 10000) < abs(nsnps - 1000), 1,
                    ifelse(abs(nsnps - 5000) < abs(nsnps - 1000), 2, 3))
